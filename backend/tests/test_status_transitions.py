...
def test_item_transitions_table_exists(engine):
    """item_transitions table is created with expected columns including from_status."""
    from sqlalchemy import inspect as sa_inspect

    insp = sa_inspect(engine)
    assert "item_transitions" in insp.get_table_names(), "item_transitions table must exist"
    col_names = [c["name"] for c in insp.get_columns("item_transitions")]
    assert "from_status" in col_names
    assert "to_status" in col_names
    assert "transitioned_at" in col_names
...
def test_store_items_persists_status_transitions(db, sample_project):
    ...
    db.refresh(item)
    stored = [{"from_status": t.from_status, "to_status": t.to_status,
               "transitioned_at": t.transitioned_at.isoformat()} for t in item.transitions]
    assert len(stored) == 2
    assert stored[0]["to_status"] == "To Do"
    assert stored[1]["to_status"] == "In Progress"
...
def test_recompute_updates_timestamps(db, sample_project):
    ...
    db.flush()
    db.add_all([
        ItemTransition(item_id=item.id, from_status=None, to_status="In Progress",
                       transitioned_at=pd.Timestamp("2024-03-01T00:00:00")),
        ItemTransition(item_id=item.id, from_status="In Progress", to_status="Done",
                       transitioned_at=pd.Timestamp("2024-03-10T00:00:00")),
    ])
    db.commit()
...
def test_available_statuses_returns_unique_sorted(client, db, sample_project):
    ...
    item1 = CachedItem(project_id=sample_project.id, item_key="S-1", item_type="Story",
                       workflow_timestamps={})
    item2 = CachedItem(project_id=sample_project.id, item_key="S-2", item_type="Bug",
                       workflow_timestamps={})
    db.add_all([item1, item2])
    db.flush()
    db.add_all([
        ItemTransition(item_id=item1.id, from_status=None, to_status="To Do",
                       transitioned_at=pd.Timestamp("2024-01-01T00:00:00")),
        ItemTransition(item_id=item1.id, from_status="To Do", to_status="In Progress",
                       transitioned_at=pd.Timestamp("2024-01-02T00:00:00")),
        ItemTransition(item_id=item2.id, from_status=None, to_status="To Do",
                       transitioned_at=pd.Timestamp("2024-01-03T00:00:00")),
        ItemTransition(item_id=item2.id, from_status="To Do", to_status="Done",
                       transitioned_at=pd.Timestamp("2024-01-07T00:00:00")),
    ])
    db.commit()
...
def test_available_statuses_ignores_null_transitions(client, db, sample_project):
    """Items with no transitions (legacy) are gracefully skipped."""
    from models.sync_job import CachedItem
    from models.item_transition import ItemTransition

    legacy = CachedItem(project_id=sample_project.id, item_key="LEGACY-1", item_type="Task",
                        workflow_timestamps={})
    has_transitions = CachedItem(project_id=sample_project.id, item_key="S-3", item_type="Story",
                                 workflow_timestamps={})
    db.add_all([legacy, has_transitions])
    db.flush()
    db.add(ItemTransition(item_id=has_transitions.id, from_status=None, to_status="Open",
                          transitioned_at=pd.Timestamp("2024-01-01T00:00:00")))
    db.commit()
    resp = client.get(f"/api/metrics/{sample_project.id}/available-statuses")
    assert resp.status_code == 200
...
def test_workflow_update_triggers_recompute(client, db, sample_project):
    ...
    item = CachedItem(project_id=sample_project.id, item_key="WF-1", item_type="Story",
                      workflow_timestamps={})
    db.add(item)
    db.flush()
    db.add_all([
        ItemTransition(item_id=item.id, from_status=None, to_status="Backlog",
                       transitioned_at=pd.Timestamp("2024-01-01T00:00:00")),
        ItemTransition(item_id=item.id, from_status="Backlog", to_status="In Progress",
                       transitioned_at=pd.Timestamp("2024-01-05T00:00:00")),
    ])
    db.commit()
